...
##             raise RuntimeError("argument has to be a function or a string")
    ...
    def updateCoords(self, coords, ind=None):
        ...
        assert len(coords)==len(self.data), "Wrong number of atoms in this conformation."
    ...
    def addCharges(self, chargeSet, newCharges):
        ...
        for a, c in zip_longest(self.data, newCharges):
...
class AtomSetSelector(TreeNodeSetSelector):
    ...
    #sargis 10/06/2009 - added support for nucleic acids selection
    #Nucleic Acids(NA) part is taken from Jmol
    # wiki http://wiki.jmol.org/index.php?title=AtomSets#Parts_in_nucleic_acids
    NAatomList = {}
    NAatomList['backbone'] ={}.fromkeys(
...
##         #print 'Molecule del'
    ...
    def getFilename(self):
        ...
        if parser is None:
            ...
            import os
            return os.path.basename(parser.filename)
    ...
    def mergeNPH(self, ats=None, deleteAtoms=True):
...
"""
    ...
    def buildBondsBhtree(self):
        ...
        ## cutoff of 1.1 does not connect x-O-O-x with bondOrderRadius=0.64
    ...
    def get_nonbonded_atoms(self, verbose=False):
        atoms = self.allAtoms
